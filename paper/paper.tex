...
existence and mass gap, where it had been isolated as an explicitly
open, named proposition in a public repository; the present paper is self-contained
...
\texttt{RootedChildFactorialCatalanIdentity}, as an explicitly open, named
proposition in a public repository, so that downstream theorems could assume
it honestly until an unconditional, machine-checked proof against the
repository's own definitions was supplied.  This paper supplies that proof.
...
\begin{enumerate}
  ...
  \item It reports a complete formalization of the proof in
    ...
    against the pinned toolchain of the repository and checked by
  ...
  \item It documents the reproducibility artifacts: the repository, the
    closing pull request, the exact commit, and a pinned
...
\emph{\texttt{RootedChildFactorialCatalanIdentity}} of the repository.
\end{corollary}
...
Mathlib~\cite{mathlib}, against the repository's pinned toolchain
...
All artifacts are public in the repository
\begin{center}
...
\bibitem{Repo}
L.~Eriksson,
\emph{rooted-tree-catalan-closure} (artifact repository),
\url{https://github.com/lluiseriksson/rooted-tree-catalan-closure},
